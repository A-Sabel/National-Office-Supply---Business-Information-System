...
class InventoryView(ctk.CTkFrame):
    ...
    def _build_catalog_tab(self):
        ...
        search_bar = ctk.CTkFrame(left, fg_color=CARD_BG,
        ...
        ctk.CTkEntry(
            left_tools, textvariable=self._search_var,
            placeholder_text="Search parts…", width=250, height=34
        ).pack(side="left", padx=(0, 12))

        ctk.CTkLabel(left_tools, text="Filter:", font=FONT_BODY,
                    text_color=TEXT_MUTED).pack(side="left", padx=(0, 6))
        self._filter_var = tk.StringVar(value="All")
        self._filter_var.trace_add("write", lambda *_: self._apply_search_filter())
        ctk.CTkOptionMenu(
            left_tools, variable=self._filter_var,
            values=["All", "In Stock", "Low Stock", "Out of Stock"],
            width=140, height=34, fg_color="#f5f7fa",
            button_color=BRAND_BLUE, text_color=TEXT_DARK, font=FONT_BODY
        ).pack(side="left")
        ...
        self.refresh_btn = ctk.CTkButton(
            ...
            fg_color=BRAND_BLUE, hover_color="#2980b9",
        ...
        self.page_label = ctk.CTkLabel(
            right_tools, text="1", width=42, height=28,
            fg_color="#ffffff", text_color=TEXT_DARK, font=("Segoe UI", 12, "bold"),
        )
        ...
        # Row 2: Parts table
        tbl_frame = ctk.CTkFrame(left, fg_color=CARD_BG,
                                corner_radius=10, border_width=1, border_color=BORDER)
        ...
        self.right_panel = ctk.CTkScrollableFrame(
            self.tab_catalog, width=240, fg_color=PAGE_BG,
    ...
    def _build_suppliers_tab(self):
        ...
        # Left: form
        form = ctk.CTkFrame(
            self.tab_suppliers, width=320, fg_color=CARD_BG,
            corner_radius=12, border_width=1, border_color=BORDER,
        )
        form.grid(row=0, column=0, sticky="nsew", padx=(5, 10), pady=5)
        form.pack_propagate(False)

        ctk.CTkLabel(form, text="Supplier Details", font=FONT_SECTION,
                    text_color=TEXT_DARK).pack(anchor="w", padx=16, pady=(14, 6))
        ...
        self.save_supp_btn = ctk.CTkButton(
            btn_row, text="💾  Save Supplier", fg_color=BRAND_BLUE,
            hover_color="#2980b9", font=("Segoe UI", 11, "bold"), height=34,
            command=self._save_supplier,
        )
        self.save_supp_btn.pack(side="left", padx=(0, 8))

        ctk.CTkButton(
            btn_row, text="✕  Clear", fg_color="#f3f5f8",
            hover_color="#e0e4eb", text_color=TEXT_DARK,
        ...
        # Right: supplier list
        right = ctk.CTkFrame(
            self.tab_suppliers, fg_color=CARD_BG,
            corner_radius=12, border_width=1, border_color=BORDER,
        )
        ...
        ctk.CTkLabel(supp_hdr, text="Supplier List", font=FONT_SECTION,
        ...
        style = ttk.Style()
        style.configure("Supp.Treeview", background=CARD_BG, foreground=TEXT_DARK,
                        rowheight=34, fieldbackground=CARD_BG, font=FONT_TABLE, borderwidth=0)
        style.configure("Supp.Treeview.Heading", background=BRAND_NAVY,
                        foreground=TEXT_WHITE, font=("Segoe UI", 11, "bold"), relief="flat")
        style.map("Supp.Treeview",
                background=[("selected", "#d6e4f7")], foreground=[("selected", BRAND_NAVY)])
        ...
        self.supp_tree = ttk.Treeview(
            right, columns=supp_cols, show="headings", style="Supp.Treeview",
            yscrollcommand=supp_vsb.set, xscrollcommand=supp_hsb.set,
        )
        ...
        ctk.CTkButton(
            right, text="🗑  Delete Selected Supplier",
    ...
    def _save_supplier(self):
        ...
        if not name:
            messagebox.showwarning("Validation", "Company Name is required."); return
        try:
            conn = psycopg2.connect(**self.db_config)
            cur  = conn.cursor()
    ...
    def _clear_supplier_form(self):
        ...
        for attr in ("supp_name", "supp_contact", "supp_phone", "supp_email", "supp_address"):
            entry = getattr(self, attr, None)
            if entry: entry.delete(0, "end")
    ...
    def _on_supplier_double_click(self, event):
        ...
        self._supp_status_label.configure(text=f"Editing: {vals[1]}", text_color=ACCENT_AMBER)

    def _delete_supplier(self):
        import psycopg2
        selected = self.supp_tree.selection()
        if not selected:
            messagebox.showwarning("Selection", "Please select a supplier to delete."); return
        vals = self.supp_tree.item(selected[0])["values"]
        supplier_id, name = vals[0], vals[1]
        if not messagebox.askyesno("Confirm Delete",
            f"Delete supplier '{name}'?\nThis may affect linked purchase orders."): return
        try:
            conn = psycopg2.connect(**self.db_config)
            cur  = conn.cursor()
    ...
    def _load_supplier_list(self):
        ...
        for r in rows:
            self.supp_tree.insert("", "end", values=(
                r["supplier_id"], r["company_name"] or "",
                r["contact_person"] or "", r["phone"] or "",
    ...
    def _open_edit_part_dialog(self, sku, values):
        ...
        ctk.CTkLabel(dialog, text=f"Editing Part: {sku}",
                    font=("Segoe UI", 14, "bold"), text_color=TEXT_DARK
                    ).pack(anchor="w", padx=20, pady=(16, 4))

        ctk.CTkLabel(dialog, text="Description", font=FONT_SMALL,
        ...
        ctk.CTkLabel(dialog, text="Selling Price (₱)", font=FONT_SMALL,
                    text_color=TEXT_MUTED).pack(anchor="w", padx=20, pady=(8, 2))
        ...
        ctk.CTkLabel(dialog, text="Stock Count", font=FONT_SMALL,
                    text_color=TEXT_MUTED).pack(anchor="w", padx=20, pady=(8, 2))
        stock_entry = ctk.CTkEntry(dialog, width=380, height=34)
        stock_entry.insert(0, str(stock))
        stock_entry.pack(padx=20)

        ctk.CTkLabel(dialog, text="Low Stock Threshold (Trigger Amount)", font=FONT_SMALL,
                    text_color=TEXT_MUTED).pack(anchor="w", padx=20, pady=(8, 2))
        ...
        def save():
            ...
            # Validate
            if not new_desc:
                status_label.configure(text="✗ Description cannot be empty.", text_color=ACCENT_RED)
                return
            try:
    ...
    def _build_reorder_tab(self):
        ...
        form_panel = ctk.CTkFrame(
            self.tab_reorder, fg_color=CARD_BG,
            corner_radius=12, border_width=1, border_color=BORDER,
        )
        ...
        ctk.CTkLabel(form_panel, text="Part (SKU)", font=FONT_SMALL,
                    text_color=TEXT_MUTED).grid(row=1, column=0, padx=(15, 5), sticky="w")
        self.reorder_sku = ctk.CTkComboBox(
            form_panel, values=[r[0] for r in self._parts_rows], width=150,
            command=self._on_reorder_sku_changed,
        )
        self.reorder_sku.grid(row=2, column=0, padx=(15, 5), pady=(0, 10), sticky="ew")

        ctk.CTkLabel(form_panel, text="Supplier", font=FONT_SMALL,
                    text_color=TEXT_MUTED).grid(row=1, column=1, padx=5, sticky="w")
        self.reorder_supp = ctk.CTkComboBox(
            form_panel, values=self._fetch_suppliers(), width=200,
            command=self._on_reorder_supplier_changed,
        )
        self.reorder_supp.grid(row=2, column=1, padx=5, pady=(0, 10), sticky="ew")

        ctk.CTkLabel(form_panel, text="Qty", font=FONT_SMALL,
                    text_color=TEXT_MUTED).grid(row=1, column=2, padx=5, sticky="w")
        ...
        ctk.CTkLabel(form_panel, text="Unit Cost", font=FONT_SMALL,
                    text_color=TEXT_MUTED).grid(row=1, column=3, padx=5, sticky="w")
        self._reorder_unit_cost_var = tk.StringVar(value="₱0.00")
        ctk.CTkEntry(
            form_panel, textvariable=self._reorder_unit_cost_var,
            state="disabled", width=110, fg_color="#f5f7fa",
        ).grid(row=2, column=3, padx=5, pady=(0, 10), sticky="ew")

        ctk.CTkButton(
            form_panel, text="+ Add to List", fg_color=ACCENT_GREEN,
            hover_color="#27ae60", font=("Segoe UI", 11, "bold"), width=130,
        ...
        style = ttk.Style()
        style.configure("Reorder.Treeview", background=CARD_BG, foreground=TEXT_DARK,
                        rowheight=36, fieldbackground=CARD_BG, font=FONT_TABLE, borderwidth=0)
        style.configure("Reorder.Treeview.Heading", background=BRAND_NAVY,
                        foreground=TEXT_WHITE, font=("Segoe UI", 11, "bold"), relief="flat")
        style.map("Reorder.Treeview",
                background=[("selected", "#d6e4f7")], foreground=[("selected", BRAND_NAVY)])
        ...
        self.reorder_tree = ttk.Treeview(
            list_frame, columns=ro_cols, show="headings",
        ...
        ctk.CTkButton(
            action_bar, text="✈  Mark Selected as Shipped",
            fg_color=BRAND_BLUE, hover_color="#2980b9",
            font=("Segoe UI", 11, "bold"), height=34,
        ...
        ctk.CTkButton(
            action_bar, text="⊞  Mark Selected as Received",
            fg_color=ACCENT_GREEN, hover_color="#27ae60",
            font=("Segoe UI", 11, "bold"), height=34,
            command=self._mark_received_from_reorder,
        ).pack(side="left", padx=(0, 8))

        ctk.CTkButton(
            action_bar, text="↻  Refresh List",
            fg_color="#f3f5f8", hover_color="#e0e4eb", text_color=TEXT_DARK,
            font=("Segoe UI", 11), height=34,
    ...
    def _submit_to_reorder_list(self):
        ...
        try:
            ...
            messagebox.showwarning("Validation", "Quantity must be a positive whole number.")
            return
        ...
        try:
            ...
            cur.execute("SELECT cost FROM item_parts WHERE part_number = %s AND supplier_id = %s",
                        (sku, supplier_id))
    ...
    def _load_reorder_list(self):
        ...
        for r in rows:
            tag = "shipped" if r["status"] == "Shipped" else ("received" if r["status"] == "Received" else "")
            self.reorder_tree.insert("", "end", tags=(tag,), values=(
                r["po_id"], r["part_number"], r["description"], r["supplier_name"],
                r["quantity"], f"₱{float(r['unit_cost']):,.2f}",
                f"₱{float(r['total_cost']):,.2f}", r["status"],
            ))
    ...
    def _mark_received_from_reorder(self):
        ...
        if not selected:
            messagebox.showwarning("Selection", "Please select a reorder entry first."); return
    ...
    def _receive_po(self, po_id):
        ...
        try:
            conn = psycopg2.connect(**self.db_config)
            cur  = conn.cursor()
            cur.execute("SELECT part_number, quantity, status FROM purchase_orders WHERE po_id = %s", (po_id,))
            po = cur.fetchone()
            if not po:
                messagebox.showerror("Error", f"PO #{po_id} not found."); conn.close(); return
            part_number, quantity, current_status = po
            if current_status == "Received":
                messagebox.showwarning("Already Received", "This PO has already been received.")
                conn.close(); return
            cur.execute("UPDATE parts SET stock_count = stock_count + %s, on_order = FALSE WHERE part_number = %s",
                        (quantity, part_number))
            cur.execute("UPDATE purchase_orders SET received = TRUE, status = 'Received' WHERE po_id = %s", (po_id,))
            conn.commit(); cur.close(); conn.close()
