...
class InventoryView(ctk.CTkFrame):
    ...
    def _build_catalog_section(self):
        ...
        ctk.CTkLabel(
            hdr,
            text="Parts Catalog (4,128 items)",
            font=FONT_TITLE,
            text_color=TEXT_DARK,
        ...
        ctk.CTkLabel(left_tools, text="🔍", font=("Segoe UI", 14)).pack(
            side="left", padx=(0, 8)
        )
    ...
    def _build_parts_table(self, parent):
        """Build the parts catalog table with scrollbars."""
        # Configure ttk style
        style = ttk.Style()
        ...
        style.map("Parts.Treeview.Heading", background=[("active", "#002a7a")])
        ...
        widths = (80, 220, 100, 80, 130, 120, 60, 60)
    ...
    def _go_next_page(self):
        total_pages = max(
            1, (len(self._parts_rows) + self._page_size - 1) // self._page_size
        )
        if self._current_page < total_pages:
            self._current_page += 1
            self._refresh_parts_table_page()
