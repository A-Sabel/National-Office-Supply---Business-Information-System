...
class WeeklySalesReportView(ctk.CTkScrollableFrame):
    ...
    def __init__(
        ...
        is_navigation_visible: bool = True,
        **kwargs,
    ):
        super().__init__(
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(
            title_row, text="Weekly Sales Report", font=FONT_TITLE, text_color=TEXT_DARK
        ).pack(side="left")
    ...
    def _build_kpi_strip(self):
        ...
        for col, (key, val, label, sub, colour, icon) in enumerate(specs):
            card = self._make_kpi_card(strip, key, val, label, sub, colour, icon)
            card.grid(row=0, column=col, sticky="nsew", padx=(0 if col == 0 else 12, 0))
            self._kpi_cards[key] = card

    def _make_kpi_card(self, parent, key, value, label, subtitle, colour, icon):
        ...
        # icon row
        ctk.CTkLabel(
            inner, text=icon, font=("Segoe UI", 22), text_color="#c0c8d8"
        ).pack(anchor="w")

        # big number
        num_lbl = ctk.CTkLabel(inner, text=value, font=FONT_CARD_NUM, text_color=colour)
        num_lbl.pack(anchor="w", pady=(4, 0))

        # label + subtitle
        ctk.CTkLabel(inner, text=label, font=FONT_CARD_LBL, text_color=TEXT_DARK).pack(
            anchor="w"
        )
        ctk.CTkLabel(
            inner, text=subtitle, font=FONT_CARD_SUB, text_color=TEXT_MUTED
        ).pack(anchor="w")

        # store reference to number label for live update
        self._kpi_num_labels[key] = num_lbl
        return card
    ...
    def _update_kpi(self, rows):
        ...
        self._kpi_num_labels["total_reps"].configure(text=str(total_reps))
        self._kpi_num_labels["top_invoices"].configure(text=str(max_inv))
        self._kpi_num_labels["total_invoices"].configure(text=str(total_inv))
        self._kpi_num_labels["total_comm"].configure(text=f"₱{total_comm:,.0f}")
    ...
    def _build_filter_bar(self):
        ...
        # Search icon + entry
        ctk.CTkLabel(
            inner, text="🔍", font=("Segoe UI", 15), text_color=TEXT_MUTED
        ).pack(side="left")
        ...
        # >10 invoices toggle
        ctk.CTkLabel(
            inner, text=">10 Invoices Only:", font=FONT_BODY, text_color=TEXT_MUTED
        ).pack(side="left")
        self._high_var = tk.BooleanVar(value=False)
        ctk.CTkCheckBox(
            inner,
            text="",
            variable=self._high_var,
            width=30,
            fg_color=C_AMBER,
            hover_color="#d97706",
        ...
        self._count_lbl = ctk.CTkLabel(
            inner, text="0 records", font=FONT_SMALL, text_color=TEXT_MUTED
        )
    ...
    def _build_overview_tab(self, parent) -> ctk.CTkFrame:
        ...
        self._overview_tree.tag_configure(
            "high", foreground=C_ROW_HIGH, font=("Segoe UI", 11, "bold")
        )
        self._overview_tree.tag_configure(
            "top", foreground=C_ROW_TOP, font=("Segoe UI", 11, "bold")
        )
        self._overview_tree.tag_configure(
            "zero", foreground=C_ROW_ZERO, font=("Segoe UI", 11, "bold")
        )
        self._overview_tree.tag_configure("normal", foreground=C_ROW_NORMAL)
        self._overview_tree.tag_configure(
    ...
    def _build_ytd_tab(self, parent) -> ctk.CTkFrame:
        ...
        self._ytd_tree.tag_configure("normal", foreground=C_ROW_NORMAL)
        self._ytd_tree.tag_configure("alt", foreground=C_ROW_NORMAL, background=ROW_ALT)
        return frame
    ...
    def _make_treeview(self, parent, cols, widths, row: int) -> ttk.Treeview:
        ...
        style.map("NOS.Treeview.Heading", background=[("active", "#253d52")])

        container = ctk.CTkFrame(parent, fg_color="transparent")
        container.grid(row=row, column=0, sticky="nsew", padx=16, pady=(4, 16))
        ...
        for col, w in zip(cols, widths):
            tree.heading(
                col, text=col, command=lambda c=col: self._sort(tree, c, False)
            )
            anchor = (
                "e"
                if any(
    ...
    def load_data(self):
        rows = (
            self._fetch_from_db()
            if (self.db_config and HAS_PSYCOPG2)
    ...
    def _apply_filter(self):
        ...
        only_high = self._high_var.get() if hasattr(self, "_high_var") else False
