...
class CustomerPaymentHistoryView(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._canvas.bind("<Configure>", lambda e: self._canvas.itemconfig(self._body_id, width=e.width))
    ...
    def _load_customers(self) -> dict:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
            conn.close()
            return {r[0]: (f"C-{r[1]}", r[2]) for r in rows} if rows else dict(self._SAMPLE_CUSTOMERS)
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(
            title_row, text="Customer Payment History",
        ...
        ctk.CTkButton(
            ...
            fg_color=BRAND_NAVY, hover_color="#002a7a",
            text_color=TEXT_WHITE, font=FONT_BTN,
        ...
        ctk.CTkButton(
            ...
            fg_color="transparent", hover_color="#e8f4fd",
    ...
    def _build_summary_strip(self):
        ...
        total_received = sum(p[4] for p in self.payments)
        payment_count  = len(self.payments)
        check_count    = sum(1 for p in self.payments if p[5] == "check")
        general_count  = sum(1 for p in self.payments if p[2] is None)

        cards = [
            ("Total Received",  f"₱{total_received:,.2f}", "All payments on record",         "💰", ACCENT_GREEN),
            ("Payments Logged", str(payment_count),         "Individual transactions",         "📋", BRAND_BLUE),
            ("Paid by Check",   str(check_count),           "Check payment transactions",      "🖊️", BRAND_NAVY),
            ("General Credits", str(general_count),         "Not tied to a specific invoice",  "💳", ACCENT_AMBER),
        ]
    ...
    def _build_filter_bar(self):
        ...
        ctk.CTkEntry(
            ...
            width=300, height=36, corner_radius=6,
            fg_color="#f5f7fa", text_color=TEXT_DARK, border_color=BORDER,
        ).pack(side="left", padx=(6, 20))
        self._search_var.trace_add("write", lambda *_: self._apply_filter())

        ctk.CTkLabel(inner, text="Method:", font=FONT_BODY, text_color=TEXT_MUTED).pack(side="left")
        self._method_var = ctk.StringVar(value="All")
        ctk.CTkOptionMenu(
            inner, variable=self._method_var,
        ...
        ctk.CTkLabel(inner, text="Date From:", font=FONT_BODY, text_color=TEXT_MUTED).pack(side="left")
        self._date_from_var = ctk.StringVar()
        ctk.CTkEntry(
            inner, textvariable=self._date_from_var,
        ...
        ctk.CTkLabel(inner, text="To:", font=FONT_BODY, text_color=TEXT_MUTED).pack(side="left")
        self._date_to_var = ctk.StringVar()
        ctk.CTkEntry(
            inner, textvariable=self._date_to_var,
            placeholder_text="YYYY-MM-DD",
            width=110, height=36, corner_radius=6,
            fg_color="#f5f7fa", text_color=TEXT_DARK, border_color=BORDER,
        ...
        self._count_label = ctk.CTkLabel(
            inner, text=f"{len(self.payments)} records",
            font=FONT_SMALL, text_color=TEXT_MUTED,
        )
    ...
    def _populate(self, data: list):
        ...
        for p in data:
            pay_id, cust_no, inv_id, pay_date, amount, method, bal_after = p
    ...
    def _apply_filter(self):
        ...
        for p in self.payments:
            pay_id, cust_no, inv_id, pay_date, amount, pmeth, bal = p
            ...
            if isinstance(pay_date, str):
                ...
                pay_dt = pay_date if isinstance(pay_date, datetime.date) else pay_date.date()
            else:
                pay_dt = None

            if query and not any(
                query in str(v).lower() for v in (cust_label, company, inv_id or "", pmeth or "")
            ):
                continue
