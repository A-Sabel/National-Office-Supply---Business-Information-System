...
class SummaryCard(ctk.CTkFrame):
    def __init__(self, parent, title, value, subtitle, icon, color, **kwargs):
        ...
        ctk.CTkLabel(self, text=value, font=("Segoe UI", 28, "bold"), text_color=color).pack(anchor="w", padx=16)
        ctk.CTkLabel(self, text=title, font=FONT_BODY, text_color=TEXT_DARK).pack(anchor="w", padx=16)
        ctk.CTkLabel(self, text=subtitle, font=FONT_SMALL, text_color=TEXT_MUTED).pack(anchor="w", padx=16, pady=(0, 14))
...
class InventorySalesReportView(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(parent, fg_color=BG_PAGE, corner_radius=0, border_width=0, **kwargs)
    ...
    def _load_reorder(self):
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT p.part_no, p.description, p.count_in_stock, p.trigger_amount,
                           EXISTS (
                               SELECT 1 FROM purchase_order_line pol
                               WHERE pol.part_no = p.part_no AND pol.status = 'pending'
                           ) AS on_order,
                           EXISTS (
                               SELECT 1 FROM invoice_line il
                               JOIN  invoice i ON i.invoice_no = il.invoice_no
                               WHERE il.part_no = p.part_no AND i.is_shipped = FALSE
                           ) AS has_unshipped
                    FROM qd3_low_stock p ORDER BY p.part_no
                """)
                rows = cur.fetchall()
            conn.close()
            return rows
    ...
    def _build_summary_strip(self):
        ...
        SummaryCard(strip, "Total Parts",    str(total),     "All SKUs tracked",       "📦", BRAND_BLUE  ).grid(row=0, column=0, sticky="nsew", padx=6)
        SummaryCard(strip, "In Stock",       str(in_stock),  "Sufficient quantity",    "✅", ACCENT_GREEN).grid(row=0, column=1, sticky="nsew", padx=6)
        SummaryCard(strip, "Low / Critical", str(low_stock), "Needs reorder",          "⚠️", ACCENT_AMBER).grid(row=0, column=2, sticky="nsew", padx=6)
        SummaryCard(strip, "Out of Stock",   str(out),       "Requires urgent action", "🚨", ACCENT_RED  ).grid(row=0, column=3, sticky="nsew", padx=6)

    # ==================================================================
    # Filter bar
    # ==================================================================
    def _build_filter_bar(self):
        bar = ctk.CTkFrame(self._body, fg_color=BG_CARD, corner_radius=10, border_width=1, border_color=BORDER)
        ...
        ctk.CTkEntry(
            ...
            fg_color="#f5f7fa", text_color=TEXT_DARK, border_color=BORDER,
        ).pack(side="left", padx=(6, 20))
        self._search_var.trace_add("write", lambda *_: self._apply_filter())

        ctk.CTkLabel(inner, text="Status:", font=FONT_BODY, text_color=TEXT_MUTED).pack(side="left")
        ...
        ctk.CTkOptionMenu(
            ...
            fg_color="#f5f7fa", button_color=BRAND_BLUE, text_color=TEXT_DARK,
            command=lambda _: self._apply_filter(),
        ).pack(side="left", padx=(8, 20))

        ctk.CTkLabel(inner, text="Trigger Alert Only:", font=FONT_BODY, text_color=TEXT_MUTED).pack(side="left")
        self._trigger_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(
            inner, text="", variable=self._trigger_var, width=30,
            fg_color=ACCENT_AMBER, hover_color="#e67e22", command=self._apply_filter,
        ).pack(side="left", padx=(4, 20))

        self._count_label = ctk.CTkLabel(inner, text=f"{len(self.parts)} records", font=FONT_SMALL, text_color=TEXT_MUTED)
    ...
    def _build_tabs(self):
        ...
        tab_bar = ctk.CTkFrame(wrapper, fg_color=BG_CARD, corner_radius=10, border_width=1, border_color=BORDER, height=48)
        ...
        for key, label in [
            ...
            btn = ctk.CTkButton(
                tab_bar, text=label, width=210, height=36, corner_radius=8,
                fg_color="transparent", text_color=TEXT_MUTED, hover_color="#edf2f7",
                font=FONT_BODY, command=lambda k=key: self._switch_tab(k),
            )
    ...
    def _build_overview_tab(self, parent) -> ctk.CTkFrame:
        ...
        cols   = ("Part No.", "Description", "Sell Price", "In Stock", "Trigger", "On Order", "Status")
        ...
        self._overview_tree.tag_configure("low",   foreground="#b7950b", font=("Segoe UI", 11, "bold"))
        self._overview_tree.tag_configure("out",   foreground=ACCENT_RED, font=("Segoe UI", 11, "bold"))
    ...
    def _populate_overview(self, data):
        ...
        tag_map = {"In Stock": "ok", "Low Stock": "low", "Out of Stock": "out", "On Order": "order"}
        for p in data:
            part_no, desc, price, stock, trigger, on_order, status = p
            tree.insert("", "end",
                values=(part_no, desc, f"₱{price:.2f}", stock, trigger, "Yes" if on_order else "No", status),
    ...
    def _build_reorder_tab(self, parent) -> ctk.CTkFrame:
        ...
        cols   = ("Part No.", "Description", "In Stock", "Trigger", "On Order?", "Has Unshipped Orders", "Action Required")
        widths = (90, 220, 80, 70, 90, 150, 160)

        self._reorder_tree = self._make_treeview(frame, cols, widths, row=1)
        self._reorder_tree.tag_configure("urgent", foreground=ACCENT_RED, font=("Segoe UI", 11, "bold"))
        self._reorder_tree.tag_configure("low",    foreground="#b7950b",  font=("Segoe UI", 11))
    ...
    def _populate_reorder(self):
        ...
        if db_rows is not None:
            for part_no, desc, stock, trigger, on_order, has_unshipped in db_rows:
                if   stock == 0 and not on_order: action, tag = "🚨 Urgent – Reorder NOW", "urgent"
                elif stock == 0 and on_order:     action, tag = "⏳ Order Pending",        "low"
                elif not on_order:                action, tag = "📦 Place Reorder",        "low"
                else:                             action, tag = "⏳ Order Pending",        "low"
                tree.insert("", "end",
                    values=(part_no, desc, stock, trigger,
                            "Yes" if on_order else "No",
                            "Yes" if has_unshipped else "No", action),
                    tags=(tag,),
                )
        else:
            for p in [p for p in self.parts if p[3] <= p[4]]:
                ...
                if   stock == 0:   action, tag = "🚨 Urgent – Reorder NOW", "urgent"
                elif not on_order: action, tag = "📦 Place Reorder",        "low"
                else:              action, tag = "⏳ Order Pending",        "low"
                tree.insert("", "end",
                    values=(part_no, desc, stock, trigger,
    ...
    def _build_supplier_tab(self, parent) -> ctk.CTkFrame:
        ...
        cols   = ("Part No.", "Description", "Supplier Name", "Unit Cost", "Phone", "Best Price?")
        widths = (90, 200, 180, 90, 140, 100)

        self._supplier_tree = self._make_treeview(frame, cols, widths, row=1)
        self._supplier_tree.tag_configure("cheapest", foreground="#1e8449", font=("Segoe UI", 11, "bold"))
    ...
    def _make_treeview(self, parent, cols, widths, row: int) -> ttk.Treeview:
        ...
        style.configure("NOS.Treeview",
            background=BG_CARD, foreground=TEXT_DARK, rowheight=32,
            fieldbackground=BG_CARD, font=FONT_TABLE, borderwidth=0,
        )
        ...
        tree = ttk.Treeview(
            container, columns=cols, show="headings", style="NOS.Treeview",
        ...
        for col, w in zip(cols, widths):
            tree.heading(col, text=col, command=lambda c=col: self._sort_column(tree, c, False))
