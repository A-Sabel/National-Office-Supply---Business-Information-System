...
class _WorkerFilesPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self.tree = ttk.Treeview(
            card, columns=cols, show="headings", height=8, style="Worker.Treeview"
        )
        # stretch=True on all cols so they fill the full card width
        column_specs: list[tuple[str, int, TreeAnchor, bool]] = [
            ("Employee Name", 250, "w", True),
            ("Position", 220, "w", True),
            ("Hourly Wage", 130, "center", False),
            ("SSN", 160, "w", False),
        ]
        for col, w, anc, stretch in column_specs:
...
class _AuditPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        for name, pos, period in self.MISSING:
            ...
            ctk.CTkLabel(
                row_f, text="*", font=("Segoe UI", 18), text_color="#e74c3c"
            ).pack(side="left", padx=(10, 6))
            info = ctk.CTkFrame(row_f, fg_color="transparent")
            info.pack(side="left", fill="x", expand=True, pady=8)
            ctk.CTkLabel(
                info, text=name, font=("Segoe UI", 12, "bold"), text_color="#2c3e50"
            ).pack(anchor="w")
            ctk.CTkLabel(
                info, text=pos, font=("Segoe UI", 10), text_color="#7f8c8d"
            ).pack(anchor="w")
        ...
        ctk.CTkLabel(
            right_card,
            text="Total Pending (86 Approved)",
            font=("Segoe UI", 10),
            text_color="#5d7ab5",
        ).pack(pady=(18, 0))
        ctk.CTkLabel(
...
class _TimecardPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self._ht = ttk.Treeview(
            hist, columns=cols, show="headings", height=5, style="TC.Treeview"
        )
        history_specs: list[tuple[str, int, TreeAnchor]] = [
            ("Week", 200, "w"),
            ("Hours", 90, "center"),
            ("Gross Pay", 120, "e"),
            ("Status", 90, "center"),
        ]
        for col, w, anc in history_specs:
...
class PayrollView(ctk.CTkFrame):
    ...
    def _switch(self, key):
...

        {
            ...
            "audit": _AuditPanel,
            "timecard": _TimecardPanel,
        }[key](self._scroll).grid(row=0, column=0, sticky="nsew")
        self._scroll.grid_columnconfigure(0, weight=1)
