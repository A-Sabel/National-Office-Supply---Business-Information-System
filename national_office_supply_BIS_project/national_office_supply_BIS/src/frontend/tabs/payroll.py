...
def auto_generate_timecards(db_config):
    ...
    try:
        ...
        if inserted:
            print(f"[Payroll] Auto-generated {inserted} blank timecard(s) "
                  f"for week of {week}.")
        else:
...
class _WorkerFilesPanel(ctk.CTkFrame):
    def __init__(self, parent, db_config):
        ...
        self._toggle_all_btn = ctk.CTkButton(
            toolbar, text="👁  Show All SSNs",
            width=150, height=28,
            fg_color="#ebf3fb", hover_color="#dbe9f9",
            text_color="#1f5f9f", font=("Segoe UI", 11, "bold"),
            corner_radius=6, command=self._toggle_all_ssn)
...
class _AuditPanel(ctk.CTkFrame):
    def __init__(self, parent, db_config):
        ...
        ctk.CTkButton(right_card, text="Export to CSV",
                      height=30, fg_color="#e8f5e9", hover_color="#c8e6c9",
                      text_color="#1b5e20", font=("Segoe UI", 11),
                      corner_radius=8, command=self._export_csv
        ...
        comm_card = ctk.CTkFrame(self, fg_color="#ffffff", corner_radius=12,
                                 border_width=1, border_color="#e0e0e0")
        comm_card.grid(row=2, column=0, sticky="ew", pady=(12, 0))
        comm_card.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(2, weight=0)

        ctk.CTkLabel(comm_card, text="Sales Rep Commissions — This Week",
                     font=("Segoe UI", 13, "bold"),
                     text_color="#2c3e50").pack(anchor="w", padx=16, pady=(14, 6))

        style2 = ttk.Style()
        style2.configure("Comm.Treeview",
                         font=("Segoe UI", 11), rowheight=30,
                         background="#ffffff", fieldbackground="#ffffff",
                         borderwidth=0)
        style2.configure("Comm.Treeview.Heading",
                         font=("Segoe UI", 11, "bold"),
                         background="#2c3e50", foreground="#ffffff",
                         relief="flat")

        comm_cols = ("Sales Rep", "Invoices", "Total Sales", "Commission (5%)", "Status")
        self._comm_tree = ttk.Treeview(comm_card, columns=comm_cols,
        ...
        for col, w, anc, stretch in [
            ...
            self._comm_tree.column(col, width=w, anchor=anc, stretch=stretch)

        self._comm_tree.tag_configure("even",    background="#ffffff",
                                                 foreground="#2c3e50")
        self._comm_tree.tag_configure("odd",     background="#f7f9fb",
                                                 foreground="#2c3e50")
        self._comm_tree.tag_configure("paid",    foreground="#1e8449")
        self._comm_tree.tag_configure("pending", foreground="#d35400")

        cvsb = ttk.Scrollbar(comm_card, orient="vertical",
                             command=self._comm_tree.yview)
    ...
    def _load_commissions(self):
        ...
        for idx, r in enumerate(rows):
            ...
            self._comm_tree.insert("", "end",
                                   values=(r["employee_name"],
                                           r["invoice_count"],
        ...
        try:
            with open(filepath, "w", newline="") as f:
                ...
                writer.writerow([
                    ...
                    "Pay Type", "Gross Pay", "Commission", "Net Pay"
                ])
                for r in hourly_rows:
                    gross = float(r["gross"])
                    writer.writerow([
                        r["employee_number"], r["employee_name"],
                        r["pay_type"], f"{gross:.2f}", "0.00",
                        f"{gross:.2f}"
                    ])
                for r in rep_rows:
                    ...
                    writer.writerow([
                        r["employee_number"], r["employee_name"],
                        r["pay_type"], "0.00", f"{comm:.2f}",
                        f"{comm:.2f}"
                    ])
            messagebox.showinfo("Export Successful",
                                f"Payroll exported to:\n{filepath}")
        except Exception as e:
...
class _PayrollHistoryPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_config):
        ...
        ctk.CTkLabel(hdr, text="Payroll History",
                     font=("Segoe UI", 16, "bold"),
                     text_color="#2c3e50").grid(row=0, column=0, sticky="w")

        ctk.CTkButton(hdr, text="Refresh", width=68, height=30,
                      fg_color="#ebf3fb", hover_color="#dbe9f9",
                      text_color="#1f5f9f", font=("Segoe UI", 11),
                      command=self._load_db).grid(row=0, column=1, sticky="e")

        # ── summary strip ─────────────────────────────────────────────────────
        strip = ctk.CTkFrame(self, fg_color="#ffffff", corner_radius=12,
                             border_width=1, border_color="#e0e0e0")
        strip.grid(row=1, column=0, sticky="ew", pady=(0, 10))
        strip.grid_columnconfigure((0, 1, 2), weight=1)

        self._lbl_total  = ctk.CTkLabel(strip, text="Total Paid: —",
                                         font=("Segoe UI", 12, "bold"),
                                         text_color="#1a2d5a")
        self._lbl_total.grid(row=0, column=0, padx=16, pady=12, sticky="w")

        self._lbl_hourly = ctk.CTkLabel(strip, text="Hourly Checks: —",
                                         font=("Segoe UI", 12),
                                         text_color="#27ae60")
        self._lbl_hourly.grid(row=0, column=1, padx=16, pady=12)

        self._lbl_comm   = ctk.CTkLabel(strip, text="Commission Checks: —",
                                         font=("Segoe UI", 12),
                                         text_color="#8e44ad")
        ...
        ctk.CTkLabel(card, text="All Payment Records",
                     font=("Segoe UI", 13, "bold"),
                     text_color="#2c3e50").grid(row=0, column=0, sticky="w",
                                                padx=16, pady=(14, 6))
        ...
        style.configure("PH.Treeview",
                        ...
                        background="#ffffff", fieldbackground="#ffffff",
                        borderwidth=0)
        style.configure("PH.Treeview.Heading",
                        font=("Segoe UI", 11, "bold"),
                        background="#2c3e50", foreground="#ffffff",
                        relief="flat")
        ...
        self._tree.tag_configure("even",       background="#ffffff",
                                               foreground="#2c3e50")
        self._tree.tag_configure("odd",        background="#f7f9fb",
                                               foreground="#2c3e50")
    ...
    def _load_db(self):
        ...
        for idx, r in enumerate(rows):
            ...
            ptag = r["payment_type"]
            self._tree.insert("", "end",
                              values=(r["date_paid"].strftime("%b %d, %Y"),
                                      r["employee_name"],
                                      r["payment_type"].capitalize(),
                                      r["check_number"],
                                      f"P{r['amount_paid']:,.2f}"),
...
class PayrollView(ctk.CTkFrame):
    ...
    def _switch(self, key):
        ...
        panel = {
            ...
            "history":      lambda: _PayrollHistoryPanel(
                self._scroll, self.db_config),
        }[key]()
