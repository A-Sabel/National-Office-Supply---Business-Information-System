...
class ButtonMeta(TypedDict):
    full_text: str
    icon_only: str
    img_inactive: ctk.CTkImage | None
    img_active: ctk.CTkImage | None
...
class NavigationSidebar(ctk.CTkFrame):
    def __init__(self, parent, controller, role="Manager"):
        ...
        try:
            ...
            self.logo_label = ctk.CTkLabel(
                self.top_frame, image=self.logo_image, text=""
            )
        except Exception:
            self.logo_label = ctk.CTkLabel(
                self.top_frame,
        ...
        menu_items = [
            ...
            },
            {
                ...
                "file": "user.png",
                "emoji": "👥",
                "cmd": self.controller.show_customers,
                "roles": ["Manager", "Sales Rep"],
            },
            {
                ...
                "emoji": "🛒",
                "cmd": self.controller.show_orders,
                "roles": ["Manager", "Sales Rep"],
            },
            {
                "name": "Inventory",
                "file": "package.png",
                "emoji": "🏢",
                "cmd": self.controller.show_inventory,
                "roles": ["Manager", "Sales Rep"],
            },
            {
                "name": "Payroll",
        ...
        for item in menu_items:
            if self.role in item["roles"]:
                ...
                self._button_meta[item["name"]] = {
                    ...
                    "img_inactive": icon_inactive,
                    "img_active": icon_active,
                }
    ...
    def _load_colored_icon(self, filename, hex_color):
        ...
        try:
            ...
            colored_img = Image.composite(
                solid_color, Image.new("RGBA", img.size, (0, 0, 0, 0)), alpha_mask
            )
    ...
    def highlight_tab(self, active_name):
        for name, btn in self.buttons.items():
            ...
            if name == active_name:
                ...
                if meta["img_active"]:
                    btn.configure(image=meta["img_active"])
            else:
                # Inactive State
                btn.configure(
                    fg_color="transparent", text_color="#aeb9c4", border_width=0
                )
                if meta["img_inactive"]:
                    btn.configure(image=meta["img_inactive"])
